...
class MovableAdapter : IMovable {
    ...
    public MovableAdapter(object obj) => this.obj = obj;

    Vector Position {
        get {
            return IoC.Resolve<Vector>("pos.get", obj);
        }
        set {
            return IoC.Resolve<Vector>("pos.set", obj, value).Execute();
        }
    }

    Vector Velocity {
        get {
            return IoC.Resolve<Vector>("vel.get", obj);
        }
    }
}

string PropertyAdapter(string type, string name, bool getProperty, bool setProperty){
